...
public class ShadesmarDimensionTeleporter {
	public static Holder<PoiType> poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegisterEvent event) {
		event.register(Registries.POINT_OF_INTEREST_TYPE, registerHelper -> {
			PoiType poiType = new PoiType(ImmutableSet.copyOf(WordsOfRadianceModBlocks.SHADESMAR_DIMENSION_PORTAL.get().getStateDefinition().getPossibleStates()), 0, 1);
			registerHelper.register(ResourceLocation.parse("words_of_radiance:shadesmar_dimension_portal"), poiType);
	...
	public ShadesmarDimensionTeleporter(ServerLevel level) {
		this.level = level;
	}
			...
			for (int i3 = -1; i3 < 2; i3++) {
				for (int j3 = 0; j3 < 2; j3++) {
					for (int k3 = -1; k3 < 3; k3++) {
						BlockState blockstate1 = k3 < 0 ? WordsOfRadianceModBlocks.OATHGATE_STONE.get().defaultBlockState() : Blocks.AIR.defaultBlockState();
		...
		for (int l1 = -1; l1 < 3; l1++) {
			for (int j2 = -1; j2 < 4; j2++) {
				if (l1 == -1 || l1 == 2 || j2 == -1 || j2 == 3) {
					...
					this.level.setBlock(blockpos$mutableblockpos, WordsOfRadianceModBlocks.OATHGATE_STONE.get().defaultBlockState(), 3);
				}
			}
		}
		BlockState blockstate = WordsOfRadianceModBlocks.SHADESMAR_DIMENSION_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
